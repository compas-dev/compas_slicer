...
from compas_slicer.print_organization import PlanarPrintOrganizer, add_safety_printpoints, set_extruder_toggle
            ...
            for j, path in enumerate(layer.paths):
                ...
                if path_should_be_interrupted_at_end:
                    assert path_Falses == 1, (
                        f"On an path that should be interrupted there should be 1 extruder_toggle = "
                        f"False, instead you have {path_Falses} Falses.\n  The error came up on layer {i} out of "
                        f"total {len(slicer.layers) - 1} layers, \n path {j} out of total {len(slicer.layers[i].paths) - 1} paths, \n with {len(path_extruder_toggles)} printpoints. "
                        f"\nFilename: {filename}"
                    )
                    assert path_extruder_toggles[-1] is False, (
                        f"Last printpoint of open path does not have extruder_toggle = False. \n The error is on layer "
                        f"{i} out of total {len(slicer.layers) - 1} layers, \n path {j} of total {len(slicer.layers[i].paths) - 1} paths,\n with {len(path_extruder_toggles)} printpoints. "
                        f"\nFilename: {filename}"
                    )
                else:
                    assert path_Falses == 0, (
                        f"On an path that should NOT be interrupted there should be 0 extruder_toggle "
                        f"= False, instead you have {path_Falses} Falses.\n  The error came up on layer {i} out "
                        f"of total {len(slicer.layers) - 1} layers, \n path {j} out of total {len(slicer.layers[i].paths) - 1} paths, \n with {len(path_extruder_toggles)} "
                        f"printpoints. \nFilename: {filename}"
                    )
...
def test_planar_add_safety_printpoints_for_horizontal_layers():
    ...
    for filename in stl_to_test:
        ...
        for _i, layer_key in enumerate(pp_dict):
            for _j, path_key in enumerate(pp_dict[layer_key]):
