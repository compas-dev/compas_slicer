...
def infill_fermat_spiral(slicer, seam_parameter, layer_width, max_offsets=20, reverse=False):
    """
...
def create_spiral(path_list, t, lw):
    """
    Returns a connected spiral path from an ordered list of paths

    path_list: ordered path list
    t: parameter of segmentation starting point
    lw: layer width
    """
    ...
    for element in path_list:
        pl_layer = []
        for i, layer in enumerate(element):
            pl_pts = []
            if i < 20:
                for j, path in enumerate(layer):
                    ...
                    ## define first point in path
                    if j == 0:
                        if i != 0:
                            pt_1 = compas.geometry.closest_point_on_polyline(pl_layer[i-1][0], pl)
                            par_1 = find_polyline_closest_parameter(pl, pt_1)
                        else:
                            pt_1 = pl.point(t)
                            par_1 = t
                    else:
                        pt_1 = compas.geometry.closest_point_on_polyline(pl_pts[-1], pl)
                        par_1 = find_polyline_closest_parameter(pl, pt_1)
                    pl_pts.append(pt_1)
                    
                    ## find last point (+ lw length)
                    par_2 = (par_1 + lw / pl.length) % 1
                    pt_2 = pl.point(par_2)
                    ...
                    ## assemble all points for new polyline
                    # make sure 1.0 equals 0.0
                    par_1 = par_1 % 1 
                    
                    # condition: passing curve start/end
                    if (par_1) > par_2:
                        pts = []
                        for x in par_list:
                            if x < par_1 and x > par_2:
                                pts.append(x)
                        pts = pts[::-1]
                    
                    # condition: not passing curve start/end
                    else:
                        pts_left = []
                        pts_right = []
                        for x in par_list:
                    ...
                    for pt in pts:
                        pl_pts.append((pl.point(pt)))
                            
                    ## add last point
                    pl_pts.append(pt_2)
                
                ## add all pts in layer
                pl_layer.append(pl_pts)
        pl_object.append(pl_layer)

    ## connecting layer paths in objects
    pl_con = []
    for object in pl_object:
        pl_obj_con = []
        
        for i, layer in enumerate(object): 
            # reversing every second layer
            if i % 2 != 0:
                layer = layer[::-1]
                pl_obj_con.extend(layer)
            else:
                pl_obj_con.extend(layer)
                
        pl_con.append(pl_obj_con)
