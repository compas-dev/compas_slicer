...
def generate_brim(printpoints_dict, layer_width, number_of_brim_layers):
    ...
    for printpoint in printpoints_dict[0]:
    ...
    for i in range(number_of_brim_layers):
        ...
        for pt in clipper_points_per_contour:
            pp = PrintPoint(pt, printpoints_dict[0][0].layer_height)
            pp.parent_path = printpoints_dict[0][0].parent_path
            pp.extruder_toggle = True

            clipper_printpoints.append(pp)

    # add to first print points the new clipper_printooints
    printpoints_dict[0] = clipper_printpoints + printpoints_dict[0]
