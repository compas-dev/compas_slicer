...
__all__ = ["MeshDirectedGraph", "SegmentsDirectedGraph"]
...
class DirectedGraph:
    ...
    def __init__(self) -> None:
        ...
        )  # Nested list where adj_list[i] is a list of all the neighbors
        # of the i-th component
        self.check_that_all_nodes_found_their_connectivity()
        logger.info(f"Adjacency list: {self.adj_list}")
        self.in_degree: list[int] = (
            self.get_in_degree()
        )  # Nested list where adj_list[i] is a list of all the edges pointing
        # to the i-th node.
    ...
    def get_adjacency_list(self) -> list[list[int]]:
        ...
        ]  # adjacency list , size = len(Nodes), stores nodes' neighbors
...
class MeshDirectedGraph(DirectedGraph):
    ...
    def create_graph_nodes(self) -> None:
        ...
        for i, m in enumerate(self.all_meshes):
            self.G.add_node(i, cuts=get_existing_cut_indices(m), boundaries=get_existing_boundary_indices(m))
    ...
    def get_children_of_node(self, root: int) -> tuple[list[int], list[list[int]]]:
        ...
        for key, data in self.G.nodes(data=True):
            ...
            if (
                ...
                and is_true_mesh_adjacency(self.all_meshes, key, root)
            ):
...
class SegmentsDirectedGraph(DirectedGraph):
    ...
    def __init__(self, mesh: Mesh, segments: list[VerticalLayer], max_d_threshold: float, DATA_PATH: str) -> None:
        self.mesh = mesh
...
def is_true_mesh_adjacency(all_meshes: list[Mesh], key1: int, key2: int) -> bool:
    ...
    pts_mesh2 = [
        mesh2.vertex_coordinates(vkey)
        for vkey, data in mesh2.vertices(data=True)
        if (data["cut"] > 0 or data["boundary"] > 0)
    ]
    pts_mesh1 = [
        mesh1.vertex_coordinates(vkey)
